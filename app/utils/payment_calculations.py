...
def calculate_single_fee_installment_pending(
    fee_amount: float,
    fee_paid: float,
    installment_type: str | None,
    class_start_date: date | None,
    ...
    """
    Calculate the installment pending amount for a single fee type.
    
    Args:
        fee_amount: Total fee amount
        fee_paid: Amount paid for this fee
        installment_type: Installment type (monthly, quarterly, half_yearly, yearly)
        class_start_date: Class start date
        class_end_date: Class end date
    
    Returns:
        The installment pending amount for this fee (rounded to 2 decimal places)
    """
    # If no class dates, return total remaining
    if not class_start_date or not class_end_date:
        return round(max(0.0, fee_amount - fee_paid), 2)
    ...
    if installment_type_value == InstallmentType.MONTHLY.value:
        ...
        # Per month amount
        per_period_amount = fee_amount / total_months
        
        # Expected payment for periods passed (cumulative)
        expected_payment = per_period_amount * months_passed
        
        # Pending amount = expected payment - fee paid (but not less than 0)
        installment_pending_amount = max(0.0, expected_payment - fee_paid)
        ...
        # Per quarter amount
        per_period_amount = fee_amount / total_quarters
        ...
        # Per half-year amount
        per_period_amount = fee_amount / total_half_years
        ...
        # Default: use total remaining
        installment_pending_amount = max(0.0, fee_amount - fee_paid)
